defmodule Dobar.Old.Robot do
  ...
  def handle_info({:dialog_reaction, %Reaction{} = reaction}, state) do
    ...
    create_effects(state.supervisor, reaction)
    {:noreply, state}
  end
  ...
  defp create_effects(supervisor, message) do
    {_, effector, _, _} = get_child(supervisor, :effector)
    # {_, interface, _, _} = get_child(supervisor, :interface)

    IO.puts "effector pid is: #{inspect effector}"

    Effect.Effector.start_child(effector, message)
  end
  ...
  defp create_robot(conf) do
    ...
    children = [
      ...
      # # supervisor(Task.Supervisor, [[name: :xrx]], id: :xrx),

      # # start the effect worker
      # supervisor(Effect.Effector,
      #   [[robot: self,
      #     effectors: conf[:effects]]],
      #   id: :effector)
    ]
