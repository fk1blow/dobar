defmodule Dobar.Conversation.Root do
  ...
  def handle_cast({:evaluate_intent, intent}, %Dialog{meta: nil} = dialog) do
    IO.puts "continue dialog: #{inspect dialog}, intent: #{inspect intent}"
    topic_answer = Topic.end_topic(intent.entities)
    IO.puts "topic_answer: #{inspect topic_answer}"
    case continue_dialog(topic_answer, intent, dialog) do
      ...
      {:meta, intent} ->
        ...
        {:noreply, %{dialog | meta: pid}}
      {:error, _reason} ->
        {:noreply, dialog}
    end
  end
  def handle_cast({:evaluate_intent, intent}, %Dialog{meta: pid} = dialog) do
